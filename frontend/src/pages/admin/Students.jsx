...
export default function Students() {
  ...
  useEffect(() => {
    setLoading(true)
    studentService.listStudents(q).then(setRows).finally(() => setLoading(false))
  }, [q])

  const refresh = () => {
    setLoading(true)
    return studentService.listStudents(q).then(setRows).finally(() => setLoading(false))
  }
  ...
  const confirmDelete = async () => {
    try {
      if (confirm.row) await studentService.deleteStudent(confirm.row.student_id)
  ...
  const onSubmit = async (form) => {
    try {
      if (editing) {
        await studentService.updateStudent(editing.student_id, form)
