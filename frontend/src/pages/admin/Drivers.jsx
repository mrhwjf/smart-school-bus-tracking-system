...
export default function Drivers() {
  ...
  useEffect(() => {
    setLoading(true)
    driverService.listDrivers(q).then(setRows).finally(() => setLoading(false))
  }, [q])

  const refresh = () => {
    setLoading(true)
    return driverService.listDrivers(q).then(setRows).finally(() => setLoading(false))
  }
  ...
  const confirmDelete = async () => {
    try {
      if (confirm.row) await driverService.deleteDriver(confirm.row.user_id)
  ...
  const onSubmit = async (form) => {
    try {
      if (editing) {
        await driverService.updateDriver(editing.user_id, form)
