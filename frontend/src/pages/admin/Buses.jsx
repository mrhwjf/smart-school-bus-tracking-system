...
import * as busService from '../../api/busService'
...
export default function Buses() {
  ...
  useEffect(() => {
    setLoading(true)
    busService.listBuses(q).then(setRows).finally(() => setLoading(false))
  }, [q])

  const refresh = () => {
    setLoading(true)
    return busService.listBuses(q).then(setRows).finally(() => setLoading(false))
  }
  ...
  const confirmDelete = async () => {
    try {
      if (confirm.row) await busService.deleteBus(confirm.row.bus_id)
  ...
  const onSubmit = async (form) => {
    try {
      if (editing) {
        await busService.updateBus(editing.bus_id, form)
